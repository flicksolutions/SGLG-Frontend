...
export async function getItems({
  ...
  for (const directory of collections) {
    ...
    // let responseItem = await directus
    ...
    if (Array.isArray(responseItem)) {
      responseItem = responseItem.map((i) =>
        replaceTranslatedFields(i, directory.directory)
      );
...
export function createLabel(item) {
  let itemtype =
    typeof item.itemtype === "string" ? item.itemtype : item.itemtype.directory;
  let review,
    eventType,
    person,
    review_person,
